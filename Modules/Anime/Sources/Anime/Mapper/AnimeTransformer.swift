...
public struct AnimeTransformer: Mapper {
  ...
  public func transformResponseToEntity(request: Any?, response: AnimeResponse) -> Entity {
    ...
    animeEntity.userAmount = response.userAmount ?? 0
    animeEntity.favoriteAmount = response.favoriteAmount ?? 0
    animeEntity.nsfw = response.nsfw?.name ?? "Unknown"
    animeEntity.genre.append(objectsIn: response.genres?.map { $0.name } ?? [])
    animeEntity.mediaType = response.mediaType?.name ?? "Unknown"
    animeEntity.status = response.status?.name ?? "Unknown"
    animeEntity.episodeAmount = response.episodeAmount ?? 0
