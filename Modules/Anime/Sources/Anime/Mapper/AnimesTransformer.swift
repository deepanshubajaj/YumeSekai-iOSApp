...
public struct AnimesTransformer: Mapper {
  ...
  public func transformResponseToEntity(request: Any?, response: [AnimeDataResponse]) -> Entity {
    return response.map { result in
      ...
      animeEntity.userAmount = result.anime.userAmount ?? 0
      animeEntity.favoriteAmount = result.anime.favoriteAmount ?? 0
      animeEntity.nsfw = result.anime.nsfw?.name ?? "Unknown"
      animeEntity.genre.append(objectsIn: result.anime.genres?.map { $0.name } ?? [])
      animeEntity.mediaType = result.anime.mediaType?.name ?? "Unknown"
      animeEntity.status = result.anime.status?.name ?? "Unknown"
      animeEntity.episodeAmount = result.anime.episodeAmount ?? 0
