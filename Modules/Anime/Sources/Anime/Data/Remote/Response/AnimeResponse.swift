...
public struct AnimeResponse: Codable {
  ...
  public init(from decoder: Decoder) throws {
    ...
    self.userAmount = try? container.decodeIfPresent(Int.self, forKey: .userAmount)
    self.favoriteAmount = try? container.decodeIfPresent(Int.self, forKey: .favoriteAmount)
    ...
    self.episodeAmount = try? container.decodeIfPresent(Int.self, forKey: .episodeAmount)
    self.startSeason = try? container.decodeIfPresent(StartSeason.self, forKey: .startSeason)
    self.source = try? container.decodeIfPresent(Source.self, forKey: .source)
    self.episodeDuration = try? container.decodeIfPresent(Int.self, forKey: .episodeDuration)
    self.studios = try? container.decodeIfPresent([Studio].self, forKey: .studios)
  }
}
